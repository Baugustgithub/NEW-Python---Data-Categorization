...
KEYWORD_PATTERNS = [
    ("IT","IT Hardware & Peripherals","Computers",
     r"\b(laptop|(?:laptop|gaming)\s+notebook|desktop|workstation|(?:computer|lcd|led)\s+monitor|docking\s+station|keyboard|(?:computer|wireless|optical)\s+mouse|tablet|ipad|iphone|chromebook|server|(?:data|network|cloud)\s+storage|nas\b|san\b|network\s+switch|router|firewall|access.?point|wifi|cisco|meraki|juniper|aruba|ups|uninterruptible|barcode\s+scanner|document\s+scanner|toner|cartridge|ink.?jet|laser.?jet|projector|webcam|headset|usb\s+hub|kvm|dram|ssd\b|hard\s+drive|rack\s+mount)\b",
     "kw:it_hardware"),
    ("IT","Network / Telecom","Telecom",
     r"\b(telephone|phone\s+system|voip|pbx|handset|polycom|avaya|mitel|fiber\s+optic|cat[56]\b|ethernet\s+cable|patch\s+panel|network\s+cable|cabling|structured\s+wiring|wan\b|mpls|sdwan|sd.wan)\b",
     "kw:it_telecom"),
    ("IT","IT Software / SaaS","Software",
     r"\b((?:software|site)\s+licen[cs]e|(?:saas|software|cloud)\s+subscription|saas|software|(?:licen[cs]e|maintenance)\s+renewal|maintenance.?agree|cloud.?hosting|vmware|office.?365|azure|aws|oracle|salesforce|zoom|slack|servicenow|jira|atlassian|matlab|stata|spss|endnote|adobe|sharepoint|power\s*bi|tableau|workday|banner\s+(?:erp|system|software)|peoplesoft|blackboard|canvas\s+lms|antivirus|endpoint\s+protect|sap\b|docusign|dropbox|box\.com|github|webex|microsoft\s+365|google\s+workspace|qualtrics|smartsheet|concur)\b",
     "kw:it_software"),
    ("Research & Laboratory","Lab Consumables","Lab Supplies",
     r"\b(reagent|assay|antibod[yies]+|enzyme|(?:ph|lysis|wash)\s+buffer|pcr|qpcr|elisa|hplc|gcms|nmr|cryogenic|liquid nitrogen|cell culture|dmem|fbs|serum|pipett|microplate|well\s+plate|vial|(?:erlenmeyer|round.?bottom|culture)\s+flask|beaker|glassware|eppendorf|cuvette|syringe(?!\s+pump)|needle|slide|coverslip|petri\s+dish|agar|media|primer|oligo|plasmid|vector|(?:purified\s+)?dna\b|rna\b|protein|gel\b|membrane|lab\s+filter|tubing|disposable|sterile\s+tip|conical\s+tube|cryo\s+vial)\b",
     "kw:lab_consumables"),
    ("Research & Laboratory","Lab Equipment & Instruments","Instruments",
     r"\b(mass spec|flow cyt|sequenc|imaging system|analyzer|electrophoresis|western blot|thermocycler|autoclave|biosafety cabinet|fume hood|lyophiliz|freeze.?dry|spectrophotom|microscop|chromatograph|spectrometer|bioanalyzer|plate reader|homogenizer|sonicator|rotary evaporator|centrifuge|incubator|syringe\s+pump|peristaltic\s+pump|vacuum\s+pump|cryostat|microtome|thermal\s+cycler|gel\s+imager|shaker|vortex(?:er)?|water\s+bath|dry\s+bath|hood|laminar\s+flow)\b",
     "kw:lab_equipment"),
    ("Research & Laboratory","Lab Animals & Vivarium","Animal Research",
     r"\b(vivarium|animal\s+care|animal\s+facility|(?:lab|research)\s+(?:mouse|mice|rat|rodent)|(?:mouse|rat)\s+(?:cage|chow|feed|bedding)|animal\s+(?:feed|bedding|cage)|iacuc|veterinar|(?:transgenic|knockout)\s+(?:mouse|mice))\b",
     "kw:lab_animals"),
    # ── Clinical / Healthcare ──────────────────────────────────────────────────
    ("Clinical / Healthcare","Medical Supplies","Clinical Supplies",
     r"\b(medical supply|surgical supply|exam glove|nitrile glove|latex glove|surgical glove|surgical gown|isolation gown|catheter|bandage|gauze|suture|wound care|steri.?strip|iv\s+set|iv\s+bag|blood\s+collect|specimen|lab\s+coat|scrub[s]?|stethoscope|blood\s+pressure|otoscope|tongue\s+depress|sharps\s+container|biohazard)\b",
     "kw:clinical_supplies"),
    ("Clinical / Healthcare","Medical Equipment","Medical Devices",
     r"\b(medical\s+(?:device|equipment)|wheelchair|hospital\s+bed|exam\s+table|patient\s+monitor|infusion\s+pump|ventilator|defibrillator|ekg|ecg|ultrasound|x.?ray|mri\b|ct\s+scan|imaging\s+(?:equipment|system)|surgical\s+instrument|endoscop|anesthesia)\b",
     "kw:clinical_equipment"),
    ("Clinical / Healthcare","Pharmacy","Pharmaceuticals",
     r"\b(pharmac|prescription|medication|drug\s+(?:supply|product)|vaccine|immunization|controlled\s+substance|compounding|formulary|dispens(?:ing|ary)|rx\b)\b",
     "kw:pharmacy"),
    ("Clinical / Healthcare","Patient/Participant Payments","Stipends",
     r"\b(stipend|participant payment|patient stipend|honorarium|honoraria|subject payment|gift\s+card\s+(?:incentive|participant)|research\s+(?:incentive|compensation))\b",
     "kw:stipend"),
    ...
    ("Capital Projects & Construction","Capital Equipment Installation","Equipment Installation",
     r"\b(equipment install\w*|capital equipment install\w*|major equipment|capital install\w*|fume hood install\w*|cleanroom\s+(?:build\w*|construct\w*|install\w*|renovat\w*|project)|vivarium\s+(?:build\w*|construct\w*|install\w*|renovat\w*|project)|lab build.?out|specialized equipment)\b",
     "kw:cap_equip_install"),
    ("Facilities / MRO","Trades Services","HVAC",
     r"\b(hvac|plumb(?:ing|er)|electrical\s+contractor|conduit|(?:circuit|electrical)\s+breaker|(?:electrical|control)\s+panel|(?:service|repair)\s+call|boiler|chiller|duct(?:work)?|(?:air|hvac)\s+compressor|generator|cooling tower|thermostat|refrigerant|furnace|air\s+handler|locksmith|welder|excavat)\b",
     "kw:trades"),
    ("Facilities / MRO","Janitorial","Cleaning",
     r"\b(janitorial|custodial|(?<!clean)clean(?:ing|er)(?!room)|mop|disinfect|sanitiz|floor wax|restroom supply|trash bag|waste liner|paper towel|hand soap|broom|dustpan|vacuum\s+cleaner|carpet\s+clean)\b",
     "kw:janitorial"),
    ("Facilities / MRO","Safety & PPE","Safety",
     r"\b(ppe\b|personal protective|safety glass(?:es)?|safety goggle[s]?|hard hat|helmet|ear plug|ear muff|respirator|n95|face shield|safety vest|hi.?vis|first aid kit|fire extinguisher|aed\b|safety sign|caution tape|safety shoe|steel toe|fall\s+protect|harness|lockout.?tagout|loto\b|eye\s+wash|safety\s+shower)\b",
     "kw:safety_ppe"),
    ("Facilities / MRO","MRO Supplies","General MRO",
     r"\b(fastener|(?:hex|anchor|lag)\s+bolt|(?:lock|wing)\s+nut|(?:wood|machine|set)\s+screw|hinge|(?:spray|latex|acrylic)\s+paint|caulk|seal(?:ant)?|gasket|bearing|(?:drive|conveyor)\s+belt|valve|fitting|coupling|bracket|mounting hardware|adhesive|lubricant|cable tie|zip tie|o.?ring|clamp|strut|angle\s+iron)\b",
     "kw:mro"),
    ("Travel, Events & Hospitality","Lodging","Hotel",
     r"\b(lodging|hotel|motel|inn\b|suites|accommodation|airbnb|marriott|hilton|hyatt|hampton\s+inn|holiday\s+inn|courtyard\b|residence\s+inn)\b",
     "kw:lodging"),
    ("Travel, Events & Hospitality","Transportation","Airfare",
     r"\b(airfare|flight|airline|delta\s+air|united\s+air|american\s+airlines|southwest\s+air|amtrak|uber|lyft|per\s+diem|mileage\s+reimburse|car\s+rental|rental\s+car|ground\s+transport|parking\s+(?:fee|garage|permit)|toll[s]?|taxi|ride.?share)\b",
     "kw:transportation"),
    ("Travel, Events & Hospitality","Event / Venue","Events",
     r"\b(conference(?!\s+(?:call|bridge|line|room))|registration\s+fee|venue|event\s+space|banquet|gala|reception|award\s+ceremony|sponsorship|seminar|symposium|workshop\s+event|exhibit|tradeshow|trade\s+show|commencement|graduation\s+ceremon)\b",
     "kw:events"),
    ...
    ("Printing, Marketing & Communications","Printing","Print",
     r"\b((?<!3d\s)(?<!3d)print(?:ing)?(?!\s+(?:press|server|queue))|typeset|(?:book|document)\s+binding|poster|(?:vinyl|display)\s+banner|brochure|flyer|letterhead|business card|envelop|signage|large\s+format|wide\s+format)\b",
     "kw:printing"),
    # ── Admin & Office ─────────────────────────────────────────────────────────
    ("Admin & Office","Office Supplies","Office",
     r"\b(office supply|office supplies|staple[rs]?|paper clip|binder(?!\s+clip)|folder|(?:legal|steno)\s+pad|post.?it|dry.?erase|whiteboard|(?:permanent|dry.?erase)\s+marker|pen[cs]il|copy paper|printer paper|filing cabinet|shredder|laminator|tape\s+dispens|rubber\s+band|scissors|glue\s+stick|label\s+maker|calculator)\b",
     "kw:office_supplies"),
    ("Admin & Office","Furniture & Fixtures","Furniture",
     r"\b(office\s+(?:chair|desk|furniture)|ergonomic\s+chair|standing\s+desk|sit.?stand|bookcase|bookshelf|shelving|credenza|file\s+cabinet|cubicle|partition|conference\s+table|reception\s+desk|lounge\s+chair|task\s+chair|furniture)\b",
     "kw:furniture"),
    ("Admin & Office","Published / Books","Books & Subscriptions",
     r"\b(textbook|(?:journal|magazine|newspaper)\s+subscription|library|isbn|publication|e.?book|course\s+material|academic\s+journal|periodical|reference\s+book)\b",
     "kw:books"),
    ("Food & Catering","Food & Beverage","Catering",
     r"\b(cater(?:ing|er)|food\s+service|lunch(?:eon)?|dinner|breakfast|refreshment|beverage\s+(?:service|order)|coffee\s+service|meal\s+(?:plan|service|delivery)|snack\s+(?:service|order|box))\b",
     "kw:catering"),
    # ── Utilities & Occupancy ──────────────────────────────────────────────────
    ("Utilities & Occupancy","Electric / Gas","Utilities",
     r"\b(electric(?:ity)?|utility\s+bill|power\s+bill|gas\s+bill|water\s+bill|kilowatt|kwh|natural\s+gas|propane|fuel\s+oil|sewer|water\s+(?:usage|service))\b",
     "kw:utilities"),
    ("Utilities & Occupancy","Rent / Lease / Occupancy","Rent & Lease",
     r"\b((?:office|building|space|equipment)\s+(?:rent|lease)|lease\s+(?:payment|agreement)|occupancy|(?:monthly|annual)\s+rent|real\s+estate\s+lease|copier\s+lease|leased\s+space|sublease)\b",
     "kw:rent_lease"),
    # ── Inter-Entity / Transfers ───────────────────────────────────────────────
    ("Inter-Entity / Transfers","Subgrants & Stipends","Subawards",
     r"\b(subaward|sub.?award|subcontract|sub.?contract|subgrant|sub.?grant|subrecipient|sub.?recipient|interagenc|inter.?agency|interdepartment|inter.?department|cost\s+(?:share|transfer)|fund\s+transfer|internal\s+transfer|(?:federal|state)\s+pass.?through)\b",
     "kw:inter_entity"),
    ("Inter-Entity / Transfers","Insurance","Insurance",
     r"\b(insurance\s+(?:premium|policy|coverage)|(?:general|professional|property|liability|workers.?\s+comp)\s+insurance|risk\s+management\s+(?:fee|premium)|bonding|surety)\b",
     "kw:insurance"),
]
